                    ...
                    <tbody style={{ verticalAlign: 'middle' }}>
                        {pickingList.map((val,key) =>{
                            return(
                                ...
                                <td data-title="status"><label className="iconcheck-name mx-2">{val.o_bor_status == 1 ? 'รอการอนุมัติ': val.o_dis_status ==2 ? 'อนุมัติ': 'ไม่อนุมัติ'}<i className="fas fa-check iconcheck-name" /></label> </td>
                            </tr>
                            )
                        })}
                    </tbody>
